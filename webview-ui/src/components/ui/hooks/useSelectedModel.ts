...
export const useSelectedModel = (apiConfiguration?: ProviderSettings) => {
	const provider = apiConfiguration?.apiProvider || "chinalifepe"
...
function getSelectedModel({
	...
	switch (provider) {
		...
		case "chinalifepe": {
			const id = apiConfiguration.openAiModelId ?? defaultModelId
			const info = chinalifePEModels[id as keyof typeof chinalifePEModels]
			return { id, info }
		}
		case "doubao": {
			const id = apiConfiguration.apiModelId ?? defaultModelId
