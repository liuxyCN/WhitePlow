...
\${languageInstruction}`,
	...
	CONDENSE: {
		template: `你的任务是创建一份详细的对话摘要，请密切关注用户的明确请求和你之前的操作。
...
上下文：用于继续对话的上下文。根据当前任务，如果适用，应包括：
  ...
  5. 问题解决和分析：记录迄今为止解决的问题、已执行的分析或已识别的问题。对于投资机构的专业领域，请保留：
     ...
     - 投资决策相关的分析和建议
  6. 待处理任务和下一步：概述你被明确要求处理的所有待处理任务，以及你将采取的所有未完成工作的下一步（如适用）。在有助于清晰理解的地方包含数据点、关键指标或结构化内容。对于任何下一步，请包含来自最近对话的直接引用，准确显示你正在处理的任务以及你停止的位置。这应该是逐字逐句的，以确保任务之间上下文不丢失信息。保留任何必须遵循的特定要求、截止日期、标准或格式规范。
