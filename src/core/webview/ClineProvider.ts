	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	/** Resolvers for blocking API request body preview (settings: showApiRequestBodyBeforeSend). */
	private apiRequestPreviewResolvers = new Map<string, () => void>()
	...
	*/
	...
	private async applyWelcomeChinalifepeCodeIndexDefaults(): Promise<void> {
		...
		await this.updateGlobalState("codebaseIndexConfig", {
			...
			codebaseIndexEmbedderModelDimension:
				typeof existing["codebaseIndexEmbedderModelDimension"] === "number" &&
				(existing["codebaseIndexEmbedderModelDimension"] as number) > 0
					? (existing["codebaseIndexEmbedderModelDimension"] as number)
					: 4096,
		})
	}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			codebaseIndexConfig: {
				...
				codebaseIndexVectorStore: codebaseIndexConfig?.codebaseIndexVectorStore ?? "embedded",
				codebaseIndexQdrantUrl: codebaseIndexConfig?.codebaseIndexQdrantUrl ?? "http://localhost:6333",
				codebaseIndexEmbedderProvider: codebaseIndexConfig?.codebaseIndexEmbedderProvider ?? "chinalifepe",
	...
	> {
		...
		return {
			...
			codebaseIndexConfig: {
				...
				codebaseIndexVectorStore: stateValues.codebaseIndexConfig?.codebaseIndexVectorStore ?? "embedded",
				...
				codebaseIndexEmbedderModelId:
					stateValues.codebaseIndexConfig?.codebaseIndexEmbedderModelId ?? "qwen3-embedding",
				codebaseIndexEmbedderModelDimension:
	...
	/**
	 * Blocks until the webview dismisses the API request body preview or the wait times out.
	 */
	public waitForApiRequestPreviewDismissal(previewId: string): Promise<void> {
		return new Promise((resolve) => {
			...
			const done = () => {
				...
				if (this.apiRequestPreviewResolvers.delete(previewId)) {
					resolve()
				}
			}
			timer = setTimeout(
				() => {
					if (this.apiRequestPreviewResolvers.has(previewId)) {
						this.log(`API request body preview timed out (${previewId}); proceeding with request.`)
						done()
					}
				},
				5 * 60 * 1000,
			)
			this.apiRequestPreviewResolvers.set(previewId, done)
		})
	}

	public resolveApiRequestPreviewDismissal(previewId: string): void {
		const done = this.apiRequestPreviewResolvers.get(previewId)
		if (done) {
			done()
		}
	}
