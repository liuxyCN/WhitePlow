...
import { getChinalifePEModels } from "../../api/providers/chinalifepe"
...
function encryptWithPublicKey(data: string, publicKeyPem: string): string[] {
	...
	for (const chunk of chunks) {
		...
		const encryptedBuffer = crypto.publicEncrypt(publicKeyPem, buffer)
 ...
 * 使用 ticket 获取 apikey
 */
async function getApiKeyWithTicket(ticket: string, apiUrl: string): Promise<string> {
	...
	if (!apikeyResponse.ok) {
		const errorData = (await apikeyResponse.json().catch(() => ({}))) as { error?: string }
		if (apikeyResponse.status === 400 && errorData.error === "Authentication failed") {
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "requestOpenAiModels":
			...
			break
		case "requestChinalifePEModels":
			if (message?.values?.baseUrl && message?.values?.apiKey) {
				try {
					const chinalifePEModels = await getChinalifePEModels(
						message?.values?.baseUrl,
						message?.values?.apiKey,
					)

					provider.postMessageToWebview({ type: "chinalifePEModels", chinalifePEModels })
				} catch (error) {
					console.error("Failed to fetch ChinalifePE models:", error)
					provider.postMessageToWebview({ type: "chinalifePEModels", chinalifePEModels: [] })
				}
			}
		...
		case "chinalifePELogin": {
			...
			try {
				...
				// 使用 ticket 获取 apikey
				try {
					const apikey = await getApiKeyWithTicket(ticket, apiUrl)
					...
					const errorMsg =
						apikeyError instanceof Error ? apikeyError.message : t("mcp:chinalifepe.getApiKeyFailedGeneric")
					...
					try {
						...
						const errorMsg =
							captchaError instanceof Error
								...
								: t("mcp:chinalifepe.sendCaptchaFailedGeneric")
		...
		case "deleteMcpServer": {
			...
			try {
				...
				await provider
					.getMcpHub()
					?.deleteServer(message.serverName, message.source as "global" | "project" | "memory")
		...
		case "restartMcpServer": {
			try {
				await provider
					.getMcpHub()
					?.restartConnection(message.text!, message.source as "global" | "project" | "memory")
			} catch (error) {
				provider.log(
		...
		case "mcpGatewayUrl":
			...
			if (mcpGatewayUrlDebounceTimer) {
				clearTimeout(mcpGatewayUrlDebounceTimer)
			}
			// Debounce the refresh operation to avoid frequent updates
			mcpGatewayUrlDebounceTimer = setTimeout(async () => {
				mcpGatewayUrlDebounceTimer = null
		...
		case "importMode":
			try {
				...
				if (fileUri && fileUri[0]) {
					...
					// Filter out "memory" source as it's not supported by importModeWithRules
					const validSource = message.source === "global" ? "global" : "project"
					const result = await provider.customModesManager.importModeWithRules(yamlContent, validSource)
