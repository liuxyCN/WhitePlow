...
const urlPatternWithoutProtocol =
	/^([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}|^([0-9]{1,3}\.){3}[0-9]{1,3}(:[0-9]+)?/i
const filePathIndicators = /^[./\\]|\\|^[A-Z]:\\/i
const fileExtensions =
	/\.(pdf|txt|doc|docx|xls|xlsx|ppt|pptx|jpg|jpeg|png|gif|bmp|svg|zip|rar|tar|gz|json|xml|csv|html|htm|js|ts|jsx|tsx|py|java|cpp|c|h|md|yml|yaml|ini|conf|log|sql|db|sqlite|dll|exe|bin|sh|bat|cmd|ps1)$/i
...
async function processFiles(
	args: { inputs: string[]; [key: string]: any },
	functionType: string,
	config?: FileCoolConfig,
) {
	if (!config?.apiUrl) {
		...
		if (result && result.length > 0) {
			for (const _data of result) {
				...
				const correspondingInputFile = localInputs.find((inputFile) => {
					...
					const outputFileName = path.basename(_filename, path.extname(_filename))
...
async function main() {
	...
	const result = await processFiles({ inputs: ["./test.pdf"] }, "paddle", config)
	console.log(result)
}
